...
words <- df %>% 
  mutate(mark = str_c(ID, ". ", word, " - ", book),
         axis_x = rep(1:4, each = 106, length = n()),
         axis_y = rep(106:1, length = n())) %>% 
  filter(!is.na(mark))
...
plot_word <- words %>% 
  ...
  scale_x_continuous(limits = c(.9, 5),
                     expand = c(0, 0))+
  scale_y_continuous(limits = c(.5, 107.5),
                     expand = c(0, 0)) +
  theme_void()
...
plot_patches <- plot_circle / plot_word + plot_layout(heights = c(1, .7)) +
  plot_annotation(
    ...
    subtitle = "139 Words from 39 books by <span style='color:#019f66'>**Old Testament**</span><br>
   29 Words from 27 books by <span style='color:#cc31b6'>**New Testament**</span><br>
...
ggsave(here::here("figures", "BibleWordCircles.png"), width = 14, height = 18, units = "cm")
