...
```
  scale_fill_manual(values = c("#5e86bd", "#fb9214", "#307a49", 
                                "#ad8a2b",  "#7ed1f2", "#9e155e")) +

```{r}
words <- job_words %>% 
  group_by(character, chapter, word) %>% 
  filter(word %in% c("understanding", "integrity", "wisdom", "dignity", "trust", "pain", "righteous", "upright")) %>% 
  add_count(word) %>% 
...
p <- words %>% 
  ...
  labs(x = " ",
       y = "Frequency",
       title = "Selected Words through Chapter 4 to 42 in the Book of Job",
       subtitle = "First 3 chapters are removed; conversation between God and Angels(Satan)",
